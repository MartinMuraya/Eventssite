...
//       const bookingRes = await axios.post("https://eventssite-1.onrender.com", formData);
...
function Booking() {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      // 1️⃣ Save booking in backend
      const bookingRes = await axios.post(`${API_URL}/bookings`, formData);

      // 2️⃣ Trigger MPesa STK Push
      const mpesaRes = await axios.post(`${API_URL}/api/mpesa/stkpush`, {
        phone: formData.phone,
        amount: formData.amount,
        accountReference: `Booking-${bookingRes.data.booking._id}`,
        transactionDesc: `Payment for ${formData.eventType}`,
      ...
      setMessage("Payment prompt sent! Check your phone to complete payment.");
      console.log("STK Response:", mpesaRes.data);
    } catch (error) {
      console.error(error);
      setMessage("Something went wrong. Please try again.");
    } finally {
